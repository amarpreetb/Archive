...
{	
	...
	for (int Y = 0; Y < Buffer->Height; ++Y) 
	{
		...
		for (int X = 0; X < Buffer->Width; ++X) 
		{
			...
			uint8 Red = ((X - (BlueOffSet / 4))*(Y + (GreenOffSet / 4)) / 8);
			...
			while (GlobalRunning)
			{
				...
				*/
				...
				if (SUCCEEDED(GlobalSecondaryBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor)))
				{
					...
					DWORD TargetCursor = (PlayCursor + (SoundOutput.SecondaryBufferSize*SoundOutput.BytesPerSample)) % SoundOutput.SecondaryBufferSize;
					DWORD BytesToWrite;
